...
class CoronaVirusAdapter extends RecyclerView.Adapter<CoronaVirusAdapter.CoronaVirusAdapterholder> implements Filterable {
    ...
    public CoronaVirusAdapter(List<CoronaVirusData> product,Context view) {
        this.product = product;
        this.allproduct=new ArrayList<>(product);
        this.view=view;
    }
    ...
    Filter filter =new Filter() {
        ...
        protected void publishResults(CharSequence constraint, FilterResults results) {
            ...
            if(((Collection<? extends CoronaVirusData>) results.values).isEmpty()){
                product.addAll(allproduct);
            }
           /* if(product.isEmpty()){

                    List<CoronaVirusData> companyList;
                    Gson gson = new Gson();
                    SharedPreferences prefs =view.getSharedPreferences("MY_PREF", MODE_PRIVATE);
                    String string = prefs.getString("dataList", "");
                    Type type = new TypeToken<List<CoronaVirusData>>() {
                    }.getType();
                    product= gson.fromJson(string, type);
